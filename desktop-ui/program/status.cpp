auto Program::updateMessage() -> void {
  //This function is called every iteration of the GUI run loop. Acquiring the emulator mutex would incur a severe responsiveness penalty, so instead synchronize vblanksPerSecond using std::atomics.
  
  /// `messages` may be accessed by the emulator thread, so synchronize access with messageMutex
  messageMutex.lock();
