...
auto MCD::LD::getOutputRegisterValue(int regNum) -> n8
{
  ...
  case 0x06:
    ...
    if ((data.bit(0, 3) == 5) && ((mcd.cdd.io.status == CDD::Status::Seeking) || ((mcd.cdd.io.status == CDD::Status::Playing) && seekPerformedSinceLastFrameUpdate))) {
      data.bit(7) = 1;
      data.bit(5) = 1;
    } else {
  ...
  case 0x16:
    ...
    if (mcd.cdd.isDiscLoaded() && (currentDriveState >= 5)) {
      if (mcd.cdd.isDiscLaserdisc()) {
        auto frameNumber = zeroBasedFrameIndexFromABA(mcd.cdd.getCurrentSectorAsABA()) + 1;
...
auto MCD::LD::updateStopPointWithCurrentState() -> void {
  if (inputRegs[0x07] == 0xFF) {
    ...
    if (!mcd.cdd.isDiscLaserdisc()) {
      ...
      stopAba = mcd.cdd.abaFromTime(0, BCD::decode(stopPointRegs[(int)SeekPointReg::MinutesOrFrameM]), BCD::decode(stopPointRegs[(int)SeekPointReg::SecondsOrFrameL]), BCD::decode(stopPointRegs[(int)SeekPointReg::Frames]));
      ...
      stopAba = mcd.cdd.abaFromTime(videoTimeHours, videoTimeMinutes, videoTimeSeconds, videoTimeFrames);
    }

    // Apply the stop point to playback control
    mcd.cdd.targetStopPoint = stopAba;
    ...
    if (reachedStopPoint) {
      handleStopPointReached(mcd.cdd.getCurrentSectorAsABA());
    }
  }
}
...
auto MCD::LD::performSeekWithLatchedState() -> void {
  ...
  case SeekMode::SeekToVideoFrame: {
    ...
    mcd.cdd.seekToSector(CD::ABAtoLBA(aba), targetPauseState && !reachedStopPoint);
    seekPerformedSinceLastFrameUpdate = true;
    break;}
...
auto MCD::LD::zeroBasedFrameIndexFromABA(s32 aba, bool processLeadIn) -> s32 {
...
auto MCD::LD::abaFromZeroBasedFrameIndex(s32 frameIndex) -> s32 {
...
auto MCD::LD::handleStopPointReached(s32 aba) -> void {
...
auto MCD::LD::updateCurrentVideoFrameNumber(s32 aba) -> void {
...
auto MCD::LD::loadCurrentVideoFrameIntoBuffer() -> void {
  ...
  if (currentPlaybackMode != 0x03) {
    ...
    auto aba = mcd.cdd.getCurrentSectorAsABA() + sectorAdvanceOffset;
...
auto MCD::LD::power(bool reset, bool gateArrayReset) -> void {
...
auto MCD::LD::scanline(u32 vdpPixelBuffer[1495], u32 vcounter) -> void {
  ...
  for (size_t i = 0; i < video.FrameBufferWidth; ++i) {
    ...
    auto vdpColorPacked = vdp.screen->lookupPalette(currentPixelValue);
    ...
    unsigned int vdpGraphicsFader = convert6BitUnsignedToNormalized1616FixedPoint((backdrop ? (inputRegs[0x1B] >> 2) : (inputRegs[0x1A] >> 2)));
