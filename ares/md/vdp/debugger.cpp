auto VDP::Debugger::load(Node::Object parent) -> void {
  ...
  memory.vram->setRead([&](u32 address) -> u8 {
    return self.vram.memory[uint15(address >> 1)].byte(!(address & 1));
  });
  memory.vram->setWrite([&](u32 address, u8 data) -> void {
    self.vram.memory[uint15(address >> 1)].byte(!(address & 1)) = data;
  });
  ...
  memory.cram->setRead([&](u32 address) -> u8 {
    return self.cram.memory[uint6(address >> 1)].byte(!(address & 1));
  });
  memory.cram->setWrite([&](u32 address, u8 data) -> void {
    self.cram.memory[uint6(address >> 1)].byte(!(address & 1)) = data;
  });
}
