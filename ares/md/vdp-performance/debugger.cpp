auto VDP::Debugger::load(Node::Object parent) -> void {
  ...
  memory.vram->setRead([&](u32 address) -> u8 {
    return vdp.vram.memory[uint15(address >> 1)].byte(!(address & 1));
  });
  memory.vram->setWrite([&](u32 address, u8 data) -> void {
    vdp.vram.memory[uint15(address >> 1)].byte(!(address & 1)) = data;
  });
  ...
  memory.cram->setRead([&](u32 address) -> u8 {
    return vdp.cram.memory[uint6(address >> 1)].byte(!(address & 1));
  });
  memory.cram->setWrite([&](u32 address, u8 data) -> void {
    vdp.cram.memory[uint6(address >> 1)].byte(!(address & 1)) = data;
  });
}
