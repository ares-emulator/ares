...
auto V30MZ::getMem(Size size, u32 offset) -> n16 {
...
auto V30MZ::setMem(Size size, n16 data) -> void {
...
auto V30MZ::getReg(Size size) -> n16 {
  if(size == Byte) return *r.b[modrm.reg];
  if(size == Word) return *r.w[modrm.reg];
  unreachable;
}

auto V30MZ::setReg(Size size, n16 data) -> void {
...
auto V30MZ::getSeg() -> n16 {
  return *r.s[modrm.reg];
}

auto V30MZ::setSeg(n16 data) -> void {
  *r.s[modrm.reg] = data;
}
