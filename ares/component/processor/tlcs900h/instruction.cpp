...
auto TLCS900H::instructionRegister(R register) -> void {
  ...
  case 0x03: {//LD r,#
    ...
    if constexpr(bits ==  8) prefetch(6);
    if constexpr(bits == 16) prefetch(8);
    if constexpr(bits == 32) prefetch(12);
    return instructionLoad(register, immediate); }
  case 0x04:  //PUSH r
    if constexpr(bits ==  8) prefetch(6);
    if constexpr(bits == 16) prefetch(6);
    if constexpr(bits == 32) prefetch(8);
    return instructionPush(register);
  case 0x05:  //POP r
    if constexpr(bits ==  8) prefetch(8);
    if constexpr(bits == 16) prefetch(8);
    if constexpr(bits == 32) prefetch(12);
    return instructionPop(register);
  case 0x06:  //CPL r
    if constexpr(bits == 32) return undefined();
    prefetch(4);
  ...
  case 0x08:  //MUL rr,#
    ...
    auto immediate = fetchImmediate<T>();
    if constexpr(bits == 16) prefetch(24);
    if constexpr(bits == 32) prefetch(30);
    return instructionMultiply(register, immediate); }
  case 0x09:  //MULS rr,#
    if constexpr(bits == 32) return undefined(); else {
    auto immediate = fetchImmediate<T>();
    if constexpr(bits ==  8) prefetch(20);
    if constexpr(bits == 16) prefetch(26);
    return instructionMultiplySigned(register, immediate); }
  case 0x0a:  //DIV rr,#
    if constexpr(bits == 32) return undefined(); else {
    auto immediate = fetchImmediate<T>();
    if constexpr(bits ==  8) prefetch(30);
    if constexpr(bits == 16) prefetch(46);
    return instructionDivide(register, immediate); }
  case 0x0b:  //DIVS rr,#
    if constexpr(bits == 32) return undefined(); else {
    auto immediate = fetchImmediate<T>();
    if constexpr(bits ==  8) prefetch(36);
    if constexpr(bits == 16) prefetch(52);
  ...
  case 0x14:  //PAA r
    if constexpr(bits == 8) return undefined();
    prefetch(8);
  ...
  case 0x35 ... 0x37:
    return undefined();
  case 0x38: {//MINC1 #,r
    if constexpr(bits != 16) return undefined();
    auto immediate = fetchImmediate<n16>();
    prefetch(10);
  ...
  case 0x40 ... 0x47:  //MUL R,r
    if constexpr(bits == 32) return undefined(); else {
    if constexpr(bits ==  8) prefetch(22);
    if constexpr(bits == 16) prefetch(28);
    return instructionMultiply(toRegister3<T>(data), register); }
  case 0x48 ... 0x4f:  //MULS R,r
    if constexpr(bits == 32) return undefined(); else {
    if constexpr(bits ==  8) prefetch(18);
    if constexpr(bits == 16) prefetch(24);
...
auto TLCS900H::instructionSourceMemory(M memory) -> void {
  ...
  case 0x08 ... 0x0f:
    return undefined();
  case 0x10:  //LDI
    if constexpr(bits ==  8) { prefetch(12); return instructionLoad<T, +1>(); }
    if constexpr(bits == 16) { prefetch(12); return instructionLoad<T, +2>(); }
    return undefined();
  case 0x11:  //LDIR
    if constexpr(bits ==  8) { prefetch(2); return instructionLoadRepeat<T, +1>(); }
    if constexpr(bits == 16) { prefetch(2); return instructionLoadRepeat<T, +2>(); }
    return undefined();
  case 0x12:  //LDD
    if constexpr(bits ==  8) { prefetch(12); return instructionLoad<T, -1>(); }
    if constexpr(bits == 16) { prefetch(12); return instructionLoad<T, -2>(); }
  ...
  case 0x13:  //LDDR
    ...
    return undefined();
  case 0x14:  //CPI
    if constexpr(bits ==  8) { prefetch(10); return instructionCompare<T, +1>( A); }
    if constexpr(bits == 16) { prefetch(10); return instructionCompare<T, +2>(WA); }
    return undefined();
  case 0x15:  //CPIR
    if constexpr(bits ==  8) { prefetch(2); return instructionCompareRepeat<T, +1>( A); }
    if constexpr(bits == 16) { prefetch(2); return instructionCompareRepeat<T, +2>(WA); }
    return undefined();
  case 0x16:  //CPD
    if constexpr(bits ==  8) { prefetch(10); return instructionCompare<T, -1>( A); }
    if constexpr(bits == 16) { prefetch(10); return instructionCompare<T, -2>(WA); }
  ...
  case 0x38: {//ADD (mem),#
    if constexpr(bits == 32) return undefined();
    auto immediate = fetchImmediate<T>();
    if constexpr(bits ==  8) prefetch(10);
    if constexpr(bits == 16) prefetch(12);
  ...
  case 0x3f: {//CP (mem),#
    ...
    auto immediate = fetchImmediate<T>();
    if constexpr(bits ==  8) prefetch(8);
    if constexpr(bits == 16) prefetch(10);
    return instructionCompare(memory, immediate); }
  case 0x40 ... 0x47:  //MUL R,(mem)
    if constexpr(bits == 32) return undefined(); else {
    if constexpr(bits ==  8) prefetch(24);
    if constexpr(bits == 16) prefetch(30);
