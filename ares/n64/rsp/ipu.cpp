...
auto RSP::instructionADDIU(r32& rt, cr32& rs, s16 imm) -> void {
...
auto RSP::instructionBEQ(cr32& rs, cr32& rt, s16 imm) -> void {
  if(rs.u32 == rt.u32) branch.take(PC + 4 + (imm << 2));
}

auto RSP::instructionBGEZ(cr32& rs, s16 imm) -> void {
  if(rs.s32 >= 0) branch.take(PC + 4 + (imm << 2));
}

auto RSP::instructionBGEZAL(cr32& rs, s16 imm) -> void {
  RA.u32 = s32(PC + 8);
  if(rs.s32 >= 0) branch.take(PC + 4 + (imm << 2));
}

auto RSP::instructionBGTZ(cr32& rs, s16 imm) -> void {
  if(rs.s32 > 0) branch.take(PC + 4 + (imm << 2));
}

auto RSP::instructionBLEZ(cr32& rs, s16 imm) -> void {
  if(rs.s32 <= 0) branch.take(PC + 4 + (imm << 2));
}

auto RSP::instructionBLTZ(cr32& rs, s16 imm) -> void {
  if(rs.s32 < 0) branch.take(PC + 4 + (imm << 2));
}

auto RSP::instructionBLTZAL(cr32& rs, s16 imm) -> void {
  RA.u32 = s32(PC + 8);
  if(rs.s32 < 0) branch.take(PC + 4 + (imm << 2));
}

auto RSP::instructionBNE(cr32& rs, cr32& rt, s16 imm) -> void {
  if(rs.u32 != rt.u32) branch.take(PC + 4 + (imm << 2));
}
...
auto RSP::instructionLB(r32& rt, cr32& rs, s16 imm) -> void {
  rt.u32 = s8(dmem.readByte(rs.u32 + imm));
}

auto RSP::instructionLBU(r32& rt, cr32& rs, s16 imm) -> void {
  rt.u32 = u8(dmem.readByte(rs.u32 + imm));
}

auto RSP::instructionLH(r32& rt, cr32& rs, s16 imm) -> void {
  rt.u32 = s16(dmem.readHalfUnaligned(rs.u32 + imm));
}

auto RSP::instructionLHU(r32& rt, cr32& rs, s16 imm) -> void {
...
auto RSP::instructionLW(r32& rt, cr32& rs, s16 imm) -> void {
...
auto RSP::instructionSB(cr32& rt, cr32& rs, s16 imm) -> void {
  dmem.writeByte(rs.u32 + imm, rt.u32);
}

auto RSP::instructionSH(cr32& rt, cr32& rs, s16 imm) -> void {
...
auto RSP::instructionSLTI(r32& rt, cr32& rs, s16 imm) -> void {
  rt.u32 = rs.s32 < imm;
}

auto RSP::instructionSLTIU(r32& rt, cr32& rs, s16 imm) -> void {
  rt.u32 = rs.u32 < imm;
}
...
auto RSP::instructionSW(cr32& rt, cr32& rs, s16 imm) -> void {
