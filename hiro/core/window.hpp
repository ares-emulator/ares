...
struct mWindow : mObject {
  ...
  auto onClose(const std::function<void ()>& callback = {}) -> type&;
  auto onDrop(const std::function<void (std::vector<string>)>& callback = {}) -> type&;
  auto onKeyPress(const std::function<void (s32)>& callback = {}) -> type&;
  auto onKeyRelease(const std::function<void (s32)>& callback = {}) -> type&;
...
//private:
  struct State {
    ...
    std::function<void (s32)> onKeyPress;
    std::function<void (s32)> onKeyRelease;
