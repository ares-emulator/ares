...
auto Arcade::load(string location) -> LoadResult {
  ...
  if(document["game/board"].string() == "sega/sg1000a") {
    ...
    if(rom.empty()) return { invalidROM, "Ensure your ROM is in a MAME-compatible archive (.zip or .7z)." };
  ...
  if(document["game/board"].string() == "nintendo/aleck64") {
    ...
    if(pif.empty()) return {
      invalidROM,
      "Ensure your ROM is in a MAME-compatible archive (.zip or .7z) and that the Aleck64 pif ROM is available."
    };
